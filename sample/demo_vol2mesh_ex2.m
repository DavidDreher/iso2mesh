...
clear opt
opt.keepratio=0.1; % this option is only useful when iso2mesh.vol2mesh uses 'simplify' method
opt.radbound=3;    % set the target surface mesh element bounding sphere be <3 pixels in radius.
tic
[node,elem,face]=iso2mesh.vol2mesh(fullimg,1:size(fullimg,1),1:size(fullimg,2),1:size(fullimg,3),opt,100,1);
toc

hb=iso2mesh.plotmesh(node,face);
axis equal
view(-90.5,-72);
