...
[no,el]=iso2mesh.meshasphere([30 30 30],20,2.5);
...
[node3,elem3,face3]=iso2mesh.surf2mesh([nfull,nodesize],el,[0 0 0],[61 61 61],1,8,[30 30 30],[],[2 2 2 2 6 6 6 6]);
                             % ^- add node size as the last            ^ max volume     ^- edge sizes at the 8 
                             %    column to node                                           corners of the bounding box
[node3,elem3,face3]=iso2mesh.sortmesh(srcpos,node3,elem3,1:4,face3,1:3);  % reorder the nodes/elements 
                                                 % so that the nodes near earch order
                                                 % are more clustered in the memory
elem3(:,1:4)=iso2mesh.meshreorient(node3,elem3(:,1:4));   % reorient elements to ensure the volumns are positive
...
[node2,elem2,face2]=iso2mesh.surf2mesh([nfull,nodesize],el,[0 0 0],[61 61 61],1,2,[30 30 30],[],[1 1 1 1 5 5 5 5]);

figure; iso2mesh.plotmesh(node2,face2(:,1:3),'y>30');axis equal;

[node2,elem2,face2]=iso2mesh.sortmesh(srcpos,node2,elem2,1:4,face2,1:3);
elem2(:,1:4)=iso2mesh.meshreorient(node2,elem2(:,1:4));
...
[no2,el2]=iso2mesh.meshresample(no,el,0.2);  % down sample the sphere mesh

% using the coarse spherical surface, we generate a coarse volumetric
% mesh with maximum volume of 10

[node1,elem1,face1]=iso2mesh.surf2mesh(no2,el2,[0 0 0],[61 61 61],1,10,[30 30 30],[],1);
[node1,elem1,face1]=iso2mesh.sortmesh(srcpos,node1,elem1,1:4,face1,1:3);
elem1(:,1:4)=iso2mesh.meshreorient(node1,elem1(:,1:4));
